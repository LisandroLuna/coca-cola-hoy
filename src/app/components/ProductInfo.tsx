...
export default function ProductInfo({ name, urls }: ProductInfoProps) {
  ...
  useEffect(() => {
    const fetchPrices = async () => {
      const fetchedPrices = await Promise.all(
        urls.map(async ({ url, selector }) => {
          const res = await fetch(`/api/scrape?url=${encodeURIComponent(url)}&selector=${encodeURIComponent(selector)}`);
      ...
      setAveragePrice(fetchedPrices.reduce((a, b) => a + b, 0) / fetchedPrices.length);
    };
